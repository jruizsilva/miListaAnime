...
const mostrarPaginador = () => {
  ...
  const $template = d.getElementById(
    'templatePaginador'
  ).content
...
const obtenerAnimes = async () => {
  ...
  const url = `https://api.jikan.moe/v4/anime?page=${paginaActual}&limit=12&q=${anime}`
  try {
    const result = await fetch(url)
    const listaAnimes = await result.json()
    console.log(listaAnimes)
    mostrarBusqueda(listaAnimes.data)
  } catch (err) {
    console.log(err)
  }
}
...
d.addEventListener('click', e => {
  ...
  if (e.target.classList.contains('busqueda__favorite')) {
    ...
    const anime = animesBusqueda.find(
      anime => anime.mal_id === animeID
    )
    if (
      e.target.classList.contains(
        'busqueda__favorite--active'
      )
    ) {
